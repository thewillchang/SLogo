...
public class GridViewController implements ViewController, MainModelObserver {
    ...
    public final static Dimension SIZE =
            new Dimension(
    ...
    private void updateTurtles (List<Turtle> turtles) {
        for (Turtle turtle : turtles) {
            if (!myTurtles.contains(turtle)) {
                ...
                // turtle.getTurtle().setTranslateX(turtle.getTurtle().getTranslateX() + SIZE.width
                // / 2);
                // turtle.getTurtle().setTranslateY(turtle.getTurtle().getTranslateY() + SIZE.height
                // / 2);
            }
