...
public class CommandReferenceLibrary {
    ...
    public CommandReferenceLibrary (String language, UserDefinedCommandsModel commandsModel,
        ...
        myDirectoryListing =
                ResourceBundle.getBundle("resources.languages.DirectoryListing", Locale.US);
        myNumberArguments =
                ResourceBundle.getBundle("resources.languages.NumberArguments", Locale.US);
    ...
    private void initializeDirectoryReferences () {
        for (String directory : myDirectorySet) {
            List<String> directoryReferences =
                    Arrays.asList(myDirectoryListing.getString(directory).split("\\|"));
    ...
    private void initializeNumArgsMap () {
        for (String command : myCommandSet) {
            myCommandToNumArgsMap.put(command, Integer.valueOf(myNumberArguments.getString(command)
                                                               .split("\\s+")[0]));
        }
    }
        ...
        for (String command : myCommandSet) {
            if (!mySyntaxReference.containsKey(command)) {
                List<String> commandReferences =
                        Arrays.asList(myCommandReference.getString(command).split("\\|"));
