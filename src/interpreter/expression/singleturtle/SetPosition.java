...
public class SetPosition extends TurtleCommandExpression {
    ...
    protected void setNextTransition (SLogoResult myResult, Deque<Double> values) {
        ...
        myResult.getTransition().add(
                                     new TransitionState(PenChange.NO_CHANGE,
                                                         VisibleChange.NO_CHANGE, distance,
                                                         rotate - currentHeading, 0));
    ...
    private double getThetaValue (double deltaX, double deltaY) {
        if (deltaY < 0 && deltaX < 0) { return 90 - Math.toDegrees(Math.atan(deltaY / deltaX)); }
        if (deltaY < 0 && deltaX > 0) { return 360 - Math.toDegrees(Math.atan(-deltaX / deltaY)); }
        if (deltaY > 0 && deltaX < 0) { return 180 - Math.toDegrees(Math.atan(-deltaX / deltaY)); }
        if (deltaY > 0 && deltaX > 0) { return 270 - Math.toDegrees(Math.atan(deltaY / deltaX)); }
